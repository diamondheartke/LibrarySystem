...
dummy_borrow_data = [
    ...
    },
    {
        'book_id': 2,
        'user_id': 10668,
        'borrow_date': tomorrow.isoformat(),
        'due_date': tomorrow2.isoformat(),
        'return_date': None,
        'status': 'active'
    }
]
...
def run_test():
	try:
		...
		db.update('user_records', 'user_name', 'user_id', 'Diamond Heart Ebenyo', 10669)
		
		result = db.search("user_records", "user_id", 10669)

		print(result[0])
	except Exception as e:
		print(f'[ERROR] Error occured during test: {e}', file=sys.stderr)
	finally:
		db.close()
